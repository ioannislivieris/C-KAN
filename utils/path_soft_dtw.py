...
@jit(nopython=True)
def my_max(x, gamma):
    """Calculate the maximum element of an array and the corresponding exponential.
    ...
    Returns:
        tuple: Maximum element and corresponding exponential.
    """
    max_x = np.max(x)
...
@jit(nopython=True)
def my_min(x, gamma):
    """Calculate the minimum element of an array and the corresponding exponential.
    ...
    Returns:
        tuple: Minimum element and corresponding exponential.
    """
...
def my_max_hessian_product(p, z, gamma):
    """Calculate the product of the Hessian of the max function with a vector.

    Args:
        p (numpy.ndarray): Input vector.
...
def my_min_hessian_product(p, z, gamma):
    """Calculate the product of the Hessian of the min function with a vector.
...
def dtw_grad(theta, gamma):
    """Calculate the gradient of the DTW (Dynamic Time Warping) loss.

    Args:
        theta (numpy.ndarray): Input matrix.
        gamma (float): Gamma parameter.

    Returns:
        tuple: DTW loss, gradient, Q, and E.
    """
    m = theta.shape[0]
    n = theta.shape[1]
    V = np.zeros((m + 1, n + 1))
...
def dtw_hessian_prod(theta, Z, Q, E, gamma):
    """Calculate the product of the Hessian of the DTW loss with a vector.

    Args:
        theta (numpy.ndarray): Input matrix.
        Z (numpy.ndarray): Input vector.
        Q (numpy.ndarray): Q matrix.
        E (numpy.ndarray): E matrix.
        gamma (float): Gamma parameter.

    Returns:
        tuple: Result of the product.
    """
    m = Z.shape[0]
    n = Z.shape[1]
...
class PathDTWBatch(Function):
    ...
    def forward(ctx, D, gamma):
        """Forward pass of the PathDTWBatch function.

        Args:
            ctx (context): Context.
            D (torch.Tensor): Input tensor.
            gamma (float): Gamma parameter.
        ...
        for k in range(batch_size):  # loop over all D in the batch
    ...
    def backward(ctx, grad_output):
        """Backward pass of the PathDTWBatch function.
