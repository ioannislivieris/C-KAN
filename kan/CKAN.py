...
class CKAN(nn.Module):
    ...
    def __init__(self, Lag=None, Conv_parameters=None, width=None, grid=3, k=3, noise_scale=0.1, noise_scale_base=0.1, base_fun=torch.nn.SiLU(), symbolic_enabled=True, bias_trainable=True, grid_eps=1.0, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True,
                 device='cpu', seed=0):
        '''
        ...
        -----
            Lag: int
                Lag value (number of past time-series)
            width : list of int
        ...
        Example
        -------
        >>> model = Conv(width=[2,5,1], grid=5, k=3)
        ...
        self.Conv_parameters = Conv_parameters or {'out_channels': 3, 'kernel_size': 2, 'stride': 1, 'padding': 1}
        
        out_channels = self.Conv_parameters['out_channels']
        kernel_size = self.Conv_parameters['kernel_size']
        stride = self.Conv_parameters['stride']
        padding = self.Conv_parameters['padding']
        
        # Combining conv1d and ReLU into a sequential module
        self.Conv_layer = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=out_channels, kernel_size=kernel_size, stride=stride, padding=padding),
            nn.GELU(),
            nn.MaxPool1d(kernel_size=2)
        )
    
        n_size = self._get_conv_output((1, Lag))
        width = [n_size] + width
    ...
    # Forward pass through conv layer to get feature size
    ...
    def initialize_from_another_model(self, another_model, x):
        ...
        Example
        -------
        >>> model_coarse = CKAN(width=[2,5,1], grid=5, k=3)
        >>> model_fine = CKAN(width=[2,5,1], grid=10, k=3)
    ...
    def update_grid_from_samples(self, x):
        ...
        Example
        -------
        >>> model = CKAN(width=[2,5,1], grid=5, k=3)
    ...
    def forward(self, x):
        ...
        Example
        -------
        >>> model = CKAN(width=[2,5,3], grid=5, k=3)
        >>> x = torch.normal(0,1,size=(100,2))
        >>> model(x).shape
        torch.Size([100, 3])
        '''
        # Passing from Convolutinal Layer
        x = self.Conv_layer(x)
        x = x.flatten(1)
        ...
        for l in range(self.depth):
            ...
            input_range = (grid_reshape[:, :, -1] - grid_reshape[:, :, 0] + 1e-4).to(self.device)
            output_range = torch.mean(torch.abs(postacts), dim=0).to(self.device)
    ...
    def fix_symbolic(self, l, i, j, fun_name, fit_params_bool=True, a_range=(-10, 10), b_range=(-10, 10), verbose=True, random=False):
        ...
        >>> model = CKAN(width=[2,5,1], grid=5, k=3, noise_scale=1.)
        >>> x = torch.normal(0,1,size=(100,2))
    ...
    def lock(self, l, ids):
        ...
        Example
        -------
        >>> model = CKAN(width=[2,3,1], grid=5, k=3, noise_scale=1.)
    ...
    def plot(self, folder="./figures", beta=3, mask=False, mode="supervised", scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None):
        ...
        >>> model = CKAN(width=[2,3,1], grid=3, k=3, noise_scale=1.0)
        >>> x = torch.normal(0,1,size=(100,2))
        >>> model(x) # do a forward pass to obtain model.acts
        >>> model.plot()
        '''
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
        ...
        >>> model = CKAN(width=[2,5,1], grid=5, k=3, noise_scale=0.1, seed=0)
        ...
        if opt == "Adam":
            ...
            optimizer = LBFGS(self.parameters(), lr=lr, history_size=11, line_search_fn="strong_wolfe", tolerance_grad=1e-32, tolerance_change=1e-32, tolerance_ys=1e-32)
        ...
        def closure():
            ...
            reg_ = reg(self.acts_scale).to(device)
        ...
        for _ in pbar:
            ...
            # Get predictions
            pred = self.forward(dataset['test_input'][test_id].to(device))

            test_loss = loss_fn_eval(pred, dataset['test_label'][test_id].to(device))
            
            # Metrics            
            A = pred.cpu().detach().numpy().flatten()
            B = dataset['test_label'][test_id].cpu().detach().numpy().flatten()
            from utils.PerformanceMetrics import RegressionEvaluation
            MAE, RMSE, MAPE, SMAPE, Hausdorff_score, ramp_score, R2 = RegressionEvaluation(A, B)
            # print(MAE, RMSE, MAPE, SMAPE, Hausdorff_score, ramp_score, R2)

                        
            if _ % log == 0:
                pbar.set_description("train loss: %.2e | test loss: %.2e | RMSE: %.2e | R2: %.3f | reg: %.2e " % (train_loss.cpu().detach().numpy(), test_loss.cpu().detach().numpy(), RMSE, R2, reg_.cpu().detach().numpy()))
            ...
            results['test_Hausdorff'].append(Hausdorff_score)
            results['test_RAMP'].append(ramp_score)        
            results['reg'].append(reg_.cpu().detach().numpy())

            if save_fig and _ % save_fig_freq == 0:
                self.plot(folder=img_folder, in_vars=in_vars, out_vars=out_vars, title="Step {}".format(_), beta=beta)
                plt.savefig(img_folder + '/' + str(_) + '.jpg', bbox_inches='tight', dpi=300)
                plt.close()
    ...
    def prune(self, threshold=1e-2, mode="auto", active_neurons_id=None):
        ...
        model2 = CKAN(copy.deepcopy(self.width), self.grid, self.k, base_fun=self.base_fun)
    ...
    def symbolic_formula(self, floating_digit=2, var=None, normalizer=None, simplify=False):
        ...
        Example
        -------
        >>> model = CKAN(width=[2,5,1], grid=5, k=3, noise_scale=0.1, seed=0, grid_eps=0.02)
        >>> f = lambda x: torch.exp(torch.sin(torch.pi*x[:,[0]]) + x[:,[1]]**2)
        >>> dataset = create_dataset(f, n_var=2)
        >>> model.train(dataset, opt='LBFGS', steps=50, lamb=0.01);
        >>> model = model.prune()
